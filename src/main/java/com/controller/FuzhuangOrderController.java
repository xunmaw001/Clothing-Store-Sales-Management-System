...
//                            fuzhuangOrderEntity.setAddressId(Integer.valueOf(data.get(0)));   //收货地址 要改的
...
//                            fuzhuangOrderEntity.setFuzhuangOrderYesnoTypes(Integer.valueOf(data.get(0)));   //审核状态 要改的
//                            fuzhuangOrderEntity.setFuzhuangOrderYesnoText(data.get(0));                    //审核意见 要改的
    ...
    */
    ...
    public R add(@RequestBody FuzhuangOrderEntity fuzhuangOrder, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,fuzhuangOrder:{}",this.getClass().getName(),fuzhuangOrder.toString());
            ...
            double balance = yonghuEntity.getNewMoney() - fuzhuangEntity.getFuzhuangNewMoney()*fuzhuangOrder.getBuyNumber();//余额
            if(balance<0)
                return R.error(511,"余额不够支付");
            fuzhuangOrder.setFuzhuangOrderTypes(3); //设置订单状态为已支付
            fuzhuangOrder.setFuzhuangOrderTruePrice(fuzhuangEntity.getFuzhuangNewMoney()*fuzhuangOrder.getBuyNumber()); //设置实付价格
...
//        Integer fuzhuangOrderYesnoTypes = Integer.valueOf(String.valueOf(params.get("fuzhuangOrderYesnoTypes")));//审核状态
//        Integer fuzhuangOrderYesnoText = Integer.valueOf(String.valueOf(params.get("fuzhuangOrderYesnoText")));//审核意见
        ...
        Double zhekou =1.0;
        if(yonghuEntity.getDengjiTypes() ==2){
            zhekou =0.8;
        }
        ...
        for (Map<String, Object> map : fuzhuangs) {
           //取值
            ...
            fuzhuangOrderEntity.setAddressId(addressId);//收货地址
            ...
            fuzhuangOrderEntity.setFuzhuangOrderYesnoTypes(null);//审核状态
            fuzhuangOrderEntity.setFuzhuangOrderYesnoText(null);//审核意见 ？？？？？？
            ...
            if(fuzhuangOrderPaymentTypes == 1){//余额支付
                //计算金额
                Double money = new BigDecimal(fuzhuangEntity.getFuzhuangNewMoney()).multiply(new BigDecimal(buyNumber)).multiply(new BigDecimal(zhekou)).doubleValue();
    ...
    */
     ...
     */
    @RequestMapping("/tui")
    public R tui(Integer id){
        logger.debug("tui,,Controller:{},,ids:{}",this.getClass().getName(),id.toString());
        FuzhuangOrderEntity  fuzhuangOrderEntity = new  FuzhuangOrderEntity();
        fuzhuangOrderEntity.setId(id);
        fuzhuangOrderEntity.setFuzhuangOrderTypes(6);
        fuzhuangOrderEntity.setFuzhuangOrderYesnoTypes(1);
        boolean b =  fuzhuangOrderService.updateById( fuzhuangOrderEntity);
        if(!b){
            return R.error("收货出错");
        }
