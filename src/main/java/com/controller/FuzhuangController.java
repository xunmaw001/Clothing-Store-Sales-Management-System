    ...
    */
    ...
    public R save(@RequestBody FuzhuangEntity fuzhuang, HttpServletRequest request){
        ...
        Wrapper<FuzhuangEntity> queryWrapper = new EntityWrapper<FuzhuangEntity>()
            .eq("fuzhuang_uuid_number", fuzhuang.getFuzhuangUuidNumber())
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            FuzhuangEntity fuzhuangEntity = new FuzhuangEntity();
//                            fuzhuangEntity.setFuzhuangUuidNumber(data.get(0));                    //服装编号 要改的
//                            fuzhuangEntity.setFuzhuangName(data.get(0));                    //服装名称 要改的
//                            fuzhuangEntity.setFuzhuangPhoto("");//详情和图片
...
//                            fuzhuangEntity.setFuzhuangNewMoney(data.get(0));                    //现价 要改的
//                            fuzhuangEntity.setFuzhuangClicknum(Integer.valueOf(data.get(0)));   //热度 要改的
//                            fuzhuangEntity.setFuzhuangContent("");//详情和图片
...
//                            fuzhuangEntity.setCreateTime(date);//时间
                            ...
                            //把要查询是否重复的字段放入map中
                                //服装编号
                                if(seachFields.containsKey("fuzhuangUuidNumber")){
                                    List<String> fuzhuangUuidNumber = seachFields.get("fuzhuangUuidNumber");
                                    fuzhuangUuidNumber.add(data.get(0));//要改的
                                }else{
                                    List<String> fuzhuangUuidNumber = new ArrayList<>();
                                    fuzhuangUuidNumber.add(data.get(0));//要改的
                                    seachFields.put("fuzhuangUuidNumber",fuzhuangUuidNumber);
                                }
                        }

                        //查询是否重复
                         //服装编号
                        List<FuzhuangEntity> fuzhuangEntities_fuzhuangUuidNumber = fuzhuangService.selectList(new EntityWrapper<FuzhuangEntity>().in("fuzhuang_uuid_number", seachFields.get("fuzhuangUuidNumber")).eq("fuzhuang_delete", 1));
                        if(fuzhuangEntities_fuzhuangUuidNumber.size() >0 ){
                            ArrayList<String> repeatFields = new ArrayList<>();
                            for(FuzhuangEntity s:fuzhuangEntities_fuzhuangUuidNumber){
                                repeatFields.add(s.getFuzhuangUuidNumber());
                            }
