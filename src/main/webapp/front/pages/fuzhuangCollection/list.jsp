            ...
            <div class="ng-inmod-ini">
                    <div class="ng-fault ng-container">
                        <div class="ng-inmod-the ng-container">
                            ...
                            <span class="ng-inmod-the-cn" aos="fade-down">服装收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="fuzhuangUuidNumber" id="fuzhuangUuidNumber" placeholder="服装编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="fuzhuangName" id="fuzhuangName" placeholder="服装名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="fuzhuangTypes" id="fuzhuangTypes" lay-filter="fuzhuangTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in fuzhuangTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <i @click="deleteFuzhuangCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: -10px;font-size:20px">&#xe640;</i>
                                    <div class="ng-prslist-img" @click="jumpCheck('../fuzhuang/detail.jsp?id='+item.fuzhuangId , item.fuzhuangDelete == null?'':item.fuzhuangDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" >
                                        <a href="#" class="ng-prsliik">
                                            <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                                <img class="swiper-item" :src="item.fuzhuangPhoto?item.fuzhuangPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                            </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jumpCheck('../fuzhuang/detail.jsp?id='+item.fuzhuangId , item.fuzhuangDelete == null?'':item.fuzhuangDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" ><span>{{item.fuzhuangName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.fuzhuangNewMoney"><span style="color: red;">{{item.fuzhuangNewMoney}} RMB</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            ,deleteFuzhuangCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('fuzhuangCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function fuzhuangCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fuzhuang_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
    ...
    // 级联表的字段数据
        ...
        function fuzhuangTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fuzhuang_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.fuzhuangTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#fuzhuangUuidNumber').val() != null && jquery('#fuzhuangUuidNumber').val() != "" && jquery('#fuzhuangUuidNumber').val() != "null" ) {
                param['fuzhuangUuidNumber'] = jquery('#fuzhuangUuidNumber').val();
            }
                     
            if (jquery('#fuzhuangName').val() != null && jquery('#fuzhuangName').val() != "" && jquery('#fuzhuangName').val() != "null" ) {
                param['fuzhuangName'] = jquery('#fuzhuangName').val();
            }
                                         
            var fuzhuangTypes = $("#fuzhuangTypes").find("option:selected").val();
            if (fuzhuangTypes != null && fuzhuangTypes != "" && fuzhuangTypes != "null") {
                            param['fuzhuangTypes'] = fuzhuangTypes;
                        }
                                                                                                                                                                
            // 当前表
                                                
            //收藏只能查类型为1的
            param['fuzhuangCollectionTypes'] = 1;

            // 获取列表数据
            http.request('fuzhuangCollection/page', 'get', param, function(res) {
