        ...
        <div class="cartBox">
            ...
            <div class="order_content">
                <ul class="order_lists" v-for="(item,index) in dataList" v-bind:key="index">
                    ...
                    <li class="list_con">
                        <div class="list_img"><img :src="item.fuzhuangPhoto"></div>
                        <div class="list_text">{{item.fuzhuangName}}</div>
                    </li>
                    <li class="list_price">
                        <p class="price">{{item.fuzhuangNewMoney}} RMB</p>
                    </li>
                    ...
                    <li class="list_sum">
                        <p class="sum_price">{{(item.fuzhuangNewMoney*item.buyNumber).toFixed(2)}} RMB</p>
                    </li>
                    <li class="list_op">
    ...
    var vue = new Vue({
        ...
        methods: {
			...
			,addCartNum(index) {
				...
				layui.http.request(`fuzhuang/info/${item.fuzhuangId}`, 'get', {}, function(res) {
					// 库存限制
					if (res.data.fuzhuangKucunNumber && res.data.fuzhuangKucunNumber > 0 && res.data.fuzhuangKucunNumber  <= item.buyNumber) {
						layui.layer.msg(`服装库存不足`, {
							time: 2000,
							icon: 5
						});
						return
			...
			// 购买
			buyClick() {
				localStorage.setItem('fuzhuangs', JSON.stringify(this.dataList));
				window.location.href = '../fuzhuangOrder/confirm.jsp'
			}
        }
    });
