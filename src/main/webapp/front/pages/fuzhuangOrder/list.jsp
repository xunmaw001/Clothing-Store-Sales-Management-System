...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的服装订单 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="modal-dialog" style="z-index: 3000">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <textarea id="fuzhuangCommentbackContent" style="width: 500px;height: 300px"></textarea>
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" @click="submitFuzhuangCommentback()" class="btn btn-theme">评价</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    <li class="layui-this" @click="pageList(0)">全部服装订单</li>
                    <li v-for="item in fuzhuangOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                </ul>
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            ...
                            <td style="width: 60px;">{{item.fuzhuangName}}</td>
                            <td style="width: 250px;">
                                <img :src="item.fuzhuangPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 80px;">{{item.fuzhuangValue}}</td>
                            <td style="width: 60px;">{{item.yonghuName}}</td>
                            <td style="width: 150px;">{{item.fuzhuangOrderUuidNumber}}</td>
                            <td style="width: 150px;">{{item.buyNumber}}</td>
                            <td style="width: 150px;">{{item.fuzhuangOrderTruePrice}}</td>
                            <td style="width: 150px;">{{item.fuzhuangOrderCourierName}}</td>
                            <td style="width: 150px;">{{item.fuzhuangOrderCourierNumber}}</td>
                            <td style="width: 80px;">{{item.fuzhuangOrderValue}}</td>
                            <td style="width: 80px;">{{item.fuzhuangOrderYesnoValue}}</td>
                            <td style="width: 150px;">{{item.fuzhuangOrderYesnoText}}</td>
                            <td style="width: 80px;">{{item.fuzhuangOrderPaymentValue}}</td>
                            ...
                            <td style="width: 100px;">
                                ...
                                <button v-if="item.fuzhuangOrderTypes==3" @click="refund(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                    <i class="layui-icon">&#xe65e;</i> 退款
                                </button>


                                <button v-if="item.fuzhuangOrderTypes==4" @click="receiving(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                    <i class="layui-icon">&#xe65e;</i> 收货
                                </button>

                                <button v-if="item.fuzhuangOrderTypes==4 ||item.fuzhuangOrderTypes==5 " @click="tui(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                    <i class="layui-icon">&#xe65e;</i> 退货退款
                                </button>
                                <%--
                                // 退货退款
        function tui(id) {
            ...
            http("fuzhuangOrder/tui?id="+id, "GET",{}, (res) => {
                if(res.code == 0){
                    ...
                    alert(res.msg);
                                ...
                                <button v-if="item.fuzhuangOrderTypes==5" @click="commentback(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius btn-theme">
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            //列表查询
            ,pageList(fuzhuangOrderTypes) {
                this.fuzhuangOrderTypes = fuzhuangOrderTypes;
                // 获取列表数据
                layui.http.request('fuzhuangOrder/page', 'get', {
                    page: 1,
                    limit: this.limit,
                    fuzhuangOrderTypes: (vue.fuzhuangOrderTypes == 0)?null:vue.fuzhuangOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('fuzhuangOrder/page', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    fuzhuangOrderTypes: (this.fuzhuangOrderTypes == 0)?null:this.fuzhuangOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`fuzhuangOrder/pay?id=`+id, 'GET',{}, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
                            time: 2000,
                            icon: 6
            ...
            ,refund(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`fuzhuangOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
            ...
            ,receiving(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`fuzhuangOrder/receiving?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功收货', {
                            time: 2000,
                            icon: 6
            ...
            ,tui(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`fuzhuangOrder/tui?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功申请退货退款,请等待审核', {
                            time: 2000,
                            icon: 6
                        });
                        window.location.reload();
            ...
            ,submitFuzhuangCommentback(){
                var fuzhuangCommentbackContent = $("#fuzhuangCommentbackContent").val();
                var id = $("#id").val();

                layui.http.request("fuzhuangOrder/commentback?id="+id+"&commentbackText="+fuzhuangCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    // 当前表的字典表
        ...
        function fuzhuangOrderTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fuzhuang_order_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function fuzhuangOrderYesnoTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fuzhuang_order_yesno_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function fuzhuangOrderPaymentTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fuzhuang_order_payment_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        // 获取列表数据
        http.request('fuzhuangOrder/page', 'get',{
            page: 1,
            limit: limit,
            fuzhuangOrderTypes: (vue.fuzhuangOrderTypes == 0)?null:vue.fuzhuangOrderTypes
        }, function(res) {
            ...
            laypage.render({
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('fuzhuangOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            fuzhuangOrderType: (vue.fuzhuangOrderTypes == 0)?null:vue.fuzhuangOrderTypes
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
